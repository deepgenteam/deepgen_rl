...
class Qwen2p5VLStableDiffusion3HF(BaseModel):
    def __init__(self,
        ...
        self.projector_1 = nn.Linear(lmm_hidden_size, self.connector.config.hidden_size)
    ...
    def llm2dit(self, x):
        ...
        Args:
            x: Either a tuple of hidden states from all layers (output.hidden_states),
               or a single tensor of hidden states (last layer only).
               For backward compatibility, this method handles both formats.
        """
        # Handle tuple input (all hidden states from LLM output)
    ...
    @property
    def llm(self):
        # Qwen2_5_VLForConditionalGeneration uses 'model' attribute instead of 'language_model'
        return self.lmm

    @property
    def config(self):
        # Return lmm config to satisfy trl's prepare_deepspeed requirements
    ...
    def train(self: T, mode: bool = True) -> T:
        ...
        if not mode:
            # Temporarily disable gradient checkpointing during eval
            self.activation_checkpointing_disable()
        elif getattr(self, '_gradient_checkpointing_enabled', False):
            # Re-enable gradient checkpointing when returning to train mode
            self.activation_checkpointing_enable()
    ...
    def prepare_forward_input(self,
        ...
        # Note: get_rope_index is defined on Qwen2_5_VLForConditionalGeneration, not Qwen2_5_VLModel
    ...
    def text2image_loss(self, data_dict):
        ...
        # hidden_states = output.last_hidden_state[:, -self.num_queries:]#query
