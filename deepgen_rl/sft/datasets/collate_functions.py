...
def dpo_collate_fn(instances: Sequence[Dict[str, Any]],
                   pad_index: int = DEFAULT_PAD_TOKEN_INDEX):

    chosen_pixel_values, rejected_pixel_values, texts = [], [], []

    for example in instances:
        chosen_pixel_values.append(example.pop('chosen_pixel_values'))
        rejected_pixel_values.append(example.pop('rejected_pixel_values'))
        texts.append(example.pop('text'))
